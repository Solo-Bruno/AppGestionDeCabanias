...
namespace Libreria.LogicaAplicacion.Dto.Mapeo
{
    public class MapeoCabania
    {
        ...
        internal static CabaniaDto ToDtoCmpleto(Cabania cabania)
        {
            var ret = ToDto(cabania);
            var listaMantCab = MapeoMantenimiento.listMantenimientoDto(cabania.MisMantenimientos);

            if (ret == null)
            {
                throw new CabaniaException("No hay datos de esa cabania");
            }
