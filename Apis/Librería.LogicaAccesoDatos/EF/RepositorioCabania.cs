...
namespace Libreria.LogicaAccesoDatos.EF
{
    public class RepositorioCabania : IRepositorioCabania
    {
        ...
        public IEnumerable<Cabania> GetCabaniasPorTopes(int top1, int top2) {
        
            var ret = _db.Cabanias.Where(c => c.CantMaxPers.CantMaxPers > top1 && c.CantMaxPers.CantMaxPers < top2).ToList();

            if (ret == null) {
                throw new ArgumentException("No hay cabanias entre esos valores");
            }
        ...
        public IEnumerable<Cabania> ObtenerCabaniasFiltro(int? tipo, int monto)
        {
            try
            {
                int idTipo = (int)tipo;
                if (idTipo != null && monto != null)
                {
                   
                   // var busqueda = _db.Cabanias.AsQueryable();
                   var busqueda = _db.Cabanias.Include(cab => cab.Tipo).Where(cab => cab.TipoId == idTipo
                    && cab.Tipo.CostoHuesped < monto && cab.TieneJacuzzi == true && cab.TieneReservas == false).ToList();
                ...
                {
                    throw new Exception("No se encontro una cabania con esas especificaciones");
                }
            }
